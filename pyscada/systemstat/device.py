...
MEDIA_ROOT = (
    ...
    else "/var/www/pyscada/http/media/"
)
...
class Handler(GenericHandlerDevice):
    ...
    def write_data(self, variable_id, value, task):
        ...
        if var.systemstatvariable.information == 21:
            if var.device.systemstatdevice.system_type == 0:
                ...
                if self.connect():
                    ...
                    try:
                        ...
                        if error != "":
                            logger.warning(
                                f"Writing to variable {var} return error {error}"
                            )
                    except (
                        socket.gaierror,
                        paramiko.ssh_exception.SSHException,
            ...
            logger.warning(
                "Systemstat cannot write %s to variable id %s" % (value, variable_id)
            )
        ...
        for item in variables_dict.values():
            ...
            if item.systemstatvariable.information == 0:
                ...
                if hasattr(psutil, "disk_usage"):
                    try:
                        cmd = (
                            'async_to_sync(self._wait_for)(psutil.disk_usage, 10, "'
                            + str(item.systemstatvariable.parameter)
                            + '").percent'
                        )
                        ssh_cmd = (
                            'psutil.disk_usage("'
                ...
                try:
                    ...
                    logger.warning(
                        f"File or directory {item.systemstatvariable.parameter} not found. Cannot check if it is mounted."
                    )
                ...
                for vp in VariableProperty.objects.filter(variable=item):
                    ...
                    try:
                        os.chdir(vp.name)
                        list_dir = async_to_sync(self._wait_for)(
                            os.listdir, 10, vp.name
                        )
                        list_dir = list(filter(os.path.isfile, list_dir))
                        list_dir.sort(key=os.path.getmtime)
                    except asyncioTimeoutError:
                        VariableProperty.objects.update_property(
                            variable_property=vp, value=str("Timeout : " + vp.name)
                        )
                        continue
                    except FileNotFoundError:
                        VariableProperty.objects.update_property(
                            variable_property=vp, value=str(vp.name + " not found")
                        )
                        continue
                    except OSError as e:
                        VariableProperty.objects.update_property(
                            variable_property=vp, value=str(vp.name + " - OSError")
                        )
                        continue
                    if list_dir is None or len(list_dir) == 0:
                        VariableProperty.objects.update_property(
                            variable_property=vp, value=str("No files in " + vp.name)
                        )
                        continue
                    if len(param) == 2:
                        try:
                        ...
                        continue
                    VariableProperty.objects.update_property(
                        variable_property=vp, value=result
                    )
                value = None
                timestamp = time()
                ...
                for vp in VariableProperty.objects.filter(variable=item):
                    ...
                    try:
                        ...
                        try:
                            ...
                            err = (
                                "FTP listing directory : first argument must be IP or known FQDN (/etc/hosts), "
                                "it's : " + param[0]
                            )
                            logger.debug(err)
                            VariableProperty.objects.update_property(
                                variable_property=vp, value=err
                            )
                            continue
                    try:
                        ftp = FTP(param[0])
                        ...
                        VariableProperty.objects.update_property(
                            variable_property=vp, value=str("Connection error")
                        )
                        continue
                    except error_reply:
                        VariableProperty.objects.update_property(
                            variable_property=vp, value=str("Reply error")
                        )
                        continue
                    except error_proto:
                        VariableProperty.objects.update_property(
                            variable_property=vp, value=str("Protocol error")
                        )
                        continue
                    except OSError:
                        VariableProperty.objects.update_property(
                            variable_property=vp, value=str("Device offline ?")
                        )
                        continue
                    if list_dir is None or len(list_dir) == 0:
                        VariableProperty.objects.update_property(
                            variable_property=vp,
                            value=str("No files in ftp://" + param[0] + vp.name),
                        )
                        continue
                    if len(param) == 3:
                        try:
    ...
    def exec_python_cmd(self, cmd, ssh_cmd=None, ssh_prefix=""):
        ...
        if self._device.systemstatdevice.system_type == 0:
            ...
            try:
                ...
                if err != "":
                    if "ImportError: No module named 'psutil'" in err:
                        ...
                        logger.warning(
                            f"Error {err} while running on {self._device} command {cmd}"
                        )
                value = o.read().decode()
            ...
            if value == "" and err != "":
                logger.warning(
                    f"Error running remote command {cmd} on {hostname}, returns : {err}"
                )
                value = None
